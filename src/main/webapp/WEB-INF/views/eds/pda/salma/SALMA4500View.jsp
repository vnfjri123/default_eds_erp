	...
	<aside class="main-sidebar" style="padding-top: 50px;">
		...
		<section class="sidebar">
			...
			<ul class="sidebar-menu" data-widget="tree" id="documents">
				...
				<li ><a href="/eds/pda/salma/selectSALMA3500View"><i class="fa fa-circle-o"></i> 출고등록</a></li>
				<li class="active"><a href="/eds/pda/salma/selectSALMA4500View"><i class="fa fa-circle-o"></i> 반품등록</a></li>
			</ul>
		</section>
		<!-- /.sidebar -->
	</aside>
							...
							<div style="background-color: #faf9f5;border: 1px solid #dedcd7;margin-top: 5px;margin-bottom: 5px;padding: 3px;">
								...
								<form class="form-inline" role="form" name="searchForm" id="searchForm" method="post" onSubmit="return false;">
									...
									<div class="form-group input-group input-group-sm" style="margin-bottom: 0px;">
										...
										<input type="date" class="form-control" style="font-size: 18px;" name="exRetuDt" id="exRetuDt" title="반품일자">
									</div>
								</form>
								<!-- ./form -->
							</div>
					...
					<div class="row">
						<div class="col-md-12" style="padding: 5px 15px 0 15px; background-color: #ebe9e4">
							<div class="pull-left" style="padding: 5px 0 0 5px">
								<i class="fa fa-file-text-o"></i> 반품 목록
							</div>
							<div class="btn-group pull-right">
								<form class="form-inline" role="form" name="SALMA4500ButtonForm" id="SALMA4500ButtonForm" method="post" onsubmit="return false;">
									<button type="button" class="btn btn-sm btn-default" name="btnSearch" id="btnSearch" onclick="doAction('SALMA4500Sheet', 'search')"><i class="fa fa-search"></i> 조회</button>
									<button type="button" class="btn btn-sm btn-primary" name="btnInput" id="btnInput" onclick="doAction('SALMA4500Sheet', 'input')" ><i class="fa fa-plus"></i> 신규</button>
<%--									<button type="button" class="btn btn-sm btn-success" name="btnSave" id="btnSave" onclick="doAction('SALMA4500Sheet', 'save')"><i class="fa fa-save"></i> 저장</button>--%>
									<button type="button" class="btn btn-sm btn-danger" name="btnDelete" id="btnDelete" onclick="doAction('SALMA4500Sheet', 'delete')"><i class="fa fa-trash"></i> 삭제</button>
								</form>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12" style="height: 100%;" id="SALMA4500">
							<!-- 시트가 될 DIV 객체 -->
							<div id="SALMA4500SheetDIV" style="width:100%; height:100%;"></div>
						</div>
					</div>
					<!-- 그리드 영역 -->
					<div class="row">
						<div class="col-md-12" style="padding: 5px 15px 0 15px; background-color: #ebe9e4">
							...
							<div class="btn-group pull-right">
								<form class="form-inline" role="form" name="SALMA4501ButtonForm" id="SALMA4501ButtonForm" method="post" onsubmit="return false;">
									<button type="button" class="btn btn-sm btn-default" name="btnSearch1" id="btnSearch1" onclick="doAction('SALMA4501Sheet', 'search')"><i class="fa fa-search"></i> 조회</button>
<%--									<button type="button" class="btn btn-sm btn-primary" name="btnInput1" id="btnInput1" onclick="doAction('SALMA4501Sheet', 'input')" ><i class="fa fa-plus"></i> 신규</button>--%>
<%--									<button type="button" class="btn btn-sm btn-success" name="btnSave1" id="btnSave1" onclick="doAction('SALMA4501Sheet', 'save')"><i class="fa fa-save"></i> 저장</button>--%>
									<button type="button" class="btn btn-sm btn-danger" name="btnDelete1" id="btnDelete1" onclick="doAction('SALMA4501Sheet', 'delete')"><i class="fa fa-trash"></i> 삭제</button>
								</form>
							</div>
						</div>
					</div>
					<div class="row">
						<div class="col-md-12" style="height: 100%;" id="SALMA4501">
							<!-- 시트가 될 DIV 객체 -->
							<div id="SALMA4501SheetDIV" style="width:100%; height:100%;"></div>
						</div>
					</div>
				</div>
			</div>
...
<script>
	var SALMA4500Sheet, SALMA4501Sheet;

	$(document).ready(async function () {
		if(!checkStorage()) await sStorage();
		init();

		$('#searchForm #exRetuDt').on('change', function(e) {
			doAction("SALMA4500Sheet", "search");
		});

		$("#btnLotNo").click(function(e) {
			if(($("#lotNo").val()).length>0){
				doAction("SALMA4501Sheet", "input");
			}
		});
		...
		$("#lotNo").keyup(function(e) {
			if(e.keyCode == "13"){
				doAction("SALMA4501Sheet", "input");
				doAction("SALMA4501Sheet", "search");
			}
		});
	...
	function init() {
		...
		document.getElementById('exRetuDt').value = edsUtil.getToday('%Y-%m-%d');
		...
		/* 그리드 초기화 속성 설정 */
		SALMA4500Sheet = new tui.Grid({
			el: document.getElementById('SALMA4500SheetDIV'),
			scrollX: true,
			scrollY: true,
			editingEvent: 'click',
		...
		SALMA4500Sheet.setColumns([
			...
			{ header:'반품번호',		name:'exRetuNo',		width:100,		align:'center',	hidden:true },
			{ header:'반품일자',		name:'exRetuDt',		width:120,		align:'center',	hidden:true },
		...
		SALMA4501Sheet = new tui.Grid({
			el: document.getElementById('SALMA4501SheetDIV'),
			scrollX: true,
			scrollY: true,
			editingEvent: 'click',
			bodyHeight: 'fitToParent',
		...
		SALMA4501Sheet.setColumns([
			...
			{ header:'회사코드',		name:'corpCd',		minWidth:100,	align:'center',	hidden:true },
			{ header:'사업장코드',	name:'busiCd',		minWidth:100,	align:'center',	hidden:true },
			{ header:'반품번호',		name:'exRetuNo',		minWidth:100,	align:'center', hidden:true },
			...
			{ header:'반품예정일자',	name:'exDueDt',		minWidth:120,	align:'center',	hidden:true },
			{ header:'반품요청번호',	name:'exRequNo',	minWidth:100,	align:'center',	hidden:true },
		...
		SALMA4500Sheet.on('focusChange', ev => {
		...
		SALMA4501Sheet.on('focusChange', ev => {
			if(ev.rowKey != ev.prevRowKey){
				setTimeout(function(){
					$('#lotNo').focus();
				}, 100);
		...
		/* 그리드생성 */
		document.getElementById('SALMA4500').style.height = 120 + 'px';
		var height = window.innerHeight - document.getElementById('SALMA4500SheetDIV').clientHeight - 191;
		document.getElementById('SALMA4501').style.height = height + 'px';
	...
	function doAction(sheetNm, sAction) {
		if (sheetNm == 'SALMA4500Sheet') {
			switch (sAction) {
				case "search":// 조회

					SALMA4501Sheet.finishEditing(); // 데이터 초기화
					SALMA4501Sheet.clear(); // 데이터 초기화

					SALMA4500Sheet.finishEditing(); // 데이터 초기화
					SALMA4500Sheet.clear(); // 데이터 초기화
					var param = ut.serializeObject(document.querySelector("#searchForm")); //조회조건
						param.exRetuDt = edsUtil.removeMinus(document.getElementById('exRetuDt').value);
					SALMA4500Sheet.resetData(edsUtil.getAjax("/eds/pda/salma/selectSALMA4500", param)); // 데이터 set

					if(SALMA4500Sheet.getRowCount() > 0 ){
						SALMA4500Sheet.focusAt(0, 0, true);
					}
				...
				case "input":// 신규
					...
					appendedData.exRetuDt = edsUtil.getToday("%Y-%m-%d");
					...
					SALMA4500Sheet.prependRow(appendedData, { focus:true }); // 마지막 ROW 추가

					edsUtil.doSave("/eds/pda/salma/insertSALMA4500", "SALMA4500Sheet", SALMA4500Sheet);

					break;
				case "delete"://삭제
					edsUtil.doDel("/eds/pda/salma/insertSALMA4500", "SALMA4500Sheet", SALMA4500Sheet);
					break;
			}
		}else if (sheetNm == 'SALMA4501Sheet') {
			switch (sAction) {
				case "search":// 조회
					SALMA4501Sheet.finishEditing(); // 데이터 초기화
					SALMA4501Sheet.clear(); // 데이터 초기화

					var row = SALMA4500Sheet.getFocusedCell();
					var param = ut.serializeObject(document.querySelector("#searchForm")); //조회조건
					param.exRetuNo = SALMA4500Sheet.getValue(row.rowKey, "exRetuNo");// Key
					SALMA4501Sheet.resetData(edsUtil.getAjax("/eds/pda/salma/selectSALMA4501", param)); // 데이터 set

					$('#lotNo').val('');

					// 마감 처리된 메인 시트에 대한 서브 시트 처리
					edsUtil.setClosedRow(SALMA4500Sheet,SALMA4501Sheet)
					setTimeout(function () {
						$('#lotNo').focus();
					}, 100);
				...
				case "input":// 신규
					...
					var row = SALMA4500Sheet.getFocusedCell();
					if(!SALMA4500Sheet.getValue(row.rowKey, "exRetuNo")){
						toastrmessage("toast-bottom-center", "error", "반품목록 저장후 사용하세요.", "실패", 1500);
						return;
					}
					...
					if(deleteStatus != "running"){
						...
						param.exRetuNo = SALMA4500Sheet.getValue(row.rowKey, "exRetuNo");
						param.boxQty = 1;
						param.taxDivi = '01';
						param.vatDivi = '01';
						...
						$.ajax({
							url: '/eds/pda/salma/insertSALMA4504',
							headers: {
						...
						data.push(param)
						$.ajax({
							url: '/eds/pda/salma/deleteSALMA4503',
							headers: {
								'X-Requested-With': 'XMLHttpRequest'
							},
							dataType: "json",
		...
		edsPopup.util.openPopup(
				...
				function (value) {
					...
					if(this.returnValue){
						...
						doAction('SALMASALMA4501', 'search');
					}
				},
				true,
				true
