	...
	<script>
				...
				let userDate=
					`<div>	
						<div class="fas icheck-blue d-inline img-circle ">
                        	<input type="checkbox" id="`+data.submitCd+`" value='`+data.currApproverCd+`' submitNm='`+data.submitNm+`'docDivi='`+data.docDivi+`'>
		...
		async function approveDoc()
		{
			...
			for (const [key, value] of Object.entries(data)) {
				...
				obj.docDivi=document.getElementById(key).getAttribute('docDivi');
				params.push(obj);
			}
			if(params.length>0)
			{
...
<body class="h-auto"style="min-height: calc(100% - 4.25rem);">
	<div style="position:relative">
		...
		<nav class="navbar navbar-expand-sm navbar-whiht navbar-light bg-whiht fixed-top" id='navHeightBar' style>
			<a class="navbar-brand" href="#"><h3>결재수신함</h3></a>
