...
<head>
	...
	<script>
		...
		async function addSubmitEl(target)
		{
			...
			param.endDivi=document.getElementById('endDivi').value;
			param.edmsDivi='05';
		...
		async function approveDoc()
		{
			...
			if(params.length>0)
			{
				const check=await edsEdms.ccConfirmSubmit(params);
				if(check)
				{
					addSubmitEl('homeTimeLine');
				};
...
<body class="h-auto" style="min-height: calc(100% - 4.25rem);">
	<div style="position:relative">
		...
		<nav class="navbar navbar-expand-sm navbar-whiht navbar-light bg-whiht fixed-top" id='navHeightBar' style>
			<a class="navbar-brand" href="#"><h3>수신참조</h3></a>
			  ...
			  <form class="form-inline" id="searchForm"  name="searchForm" onsubmit="return false;"> 
				<div class="input-group mr-2 mb-2">
					...
					<div class="input-group-prepend" style="min-width: 200px;">
						<select class="form-control select2" style="width: 100%;" name="endDivi" id="endDivi" >
							<option value="00">미확인내역</option>
							<option value="01">확인내역</option>
						</select>
					</div>
				</div>
				...
				<div class="input-group mr-2 mb-2">
					...
					<button type="button" class="btn btn-sm btn-primary " name="btnConfirm" id="btnConfirm" onclick="approveDoc()"><i class="fa fa-"></i> 일괄확인</button>
				</div>	
			  </form>
			</div>
		  </nav>
