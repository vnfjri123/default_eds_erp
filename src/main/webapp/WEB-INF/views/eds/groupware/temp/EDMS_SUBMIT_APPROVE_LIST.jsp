...
<head>
	...
	<script>
		...
		async function init() {
			...
			/* Button 셋팅 */
			edsUtil.setButtonForm(document.querySelector("#edmsSubmitApproveGridListButtonForm"));

			/* 이벤트 셋팅 */
			//edsUtil.addChangeEvent("edmsSubmitApproveGridListForm", fn_CopyForm2edmsSubmitApproveGridList);
			await edsIframe.setParams();

			/**********************************************************************
			 * Grid Info 영역 START
			 ***********************************************************************/
			edmsSubmitApproveGridList = new tui.Grid({
				el: document.getElementById('edmsSubmitApproveGridListDIV'),
				scrollX: true,
				scrollY: true,
				editingEvent: 'click',
			...
			edmsSubmitApproveGridList.setColumns([
			...
			//disabled 셋팅
			edmsSubmitApproveGridList.disableColumn('docDivi');
			edmsSubmitApproveGridList.disableColumn('submitDt');
			/**********************************************************************
			 * Grid Info 영역 END
			 ***********************************************************************/
			 ...
			 * Grid 이벤트 영역 START
			 ***********************************************************************/
			edmsSubmitApproveGridList.on('focusChange', async ev => {
				if(ev.rowKey != ev.prevRowKey){
					const docDivi=edmsSubmitApproveGridList.getRow(ev.rowKey).docDivi;
					setTimeout(async function(){
						await lnitIframe(docDivi);
					}, 100);
				}
			});
			edmsSubmitApproveGridList.on('afterChange', ev => {
			...
			edmsSubmitApproveGridList.on('click', async ev => {
				var colNm = ev.columnName;
				var target = ev.targetType;
				var focusDataNm=edmsSubmitApproveGridList.getFocusedCell().columnName;
				if((focusDataNm === 'docDivi')){
					edmsSubmitApproveGridList.finishEditing();
				}
			});
			/**********************************************************************
			 * Grid 이벤트 영역 END
			...
			/* 그리드생성 */
			/* 조회 */
			doAction("edmsSubmitApproveGridList", "search");
		}
		...
		async function doAction(sheetNm, sAction) {
			if (sheetNm == 'edmsSubmitApproveGridList') {
				switch (sAction) {
					case "search":// 조회
						edmsSubmitApproveGridList.clear(); // 데이터 초기화
						var param = ut.serializeObject(document.querySelector("#searchForm")); //조회조건
						await edmsSubmitApproveGridList.resetData(edsUtil.getAjax("/EDMS_SUBMIT_LIST/selectSubmitApproveList", param)); // 데이터 set
						if(edmsSubmitApproveGridList.getRowCount() > 0 ){
							edmsSubmitApproveGridList.focusAt(0, 0, true);
						}
						break;
		...
		$("#iframSubmit").ready(function() {
			...
			window.addEventListener("message", function(message) {
				...
				{
					var row = edmsSubmitApproveGridList.getFocusedCell();
					const data= edmsSubmitApproveGridList.getRow(row.rowKey);//그리드 기준 조회조건
			...
			document.getElementById("btnConfirm").addEventListener("click", async function(message) {
				const rowKey=edmsSubmitApproveGridList.getFocusedCell().rowKey;
				const data=edmsSubmitApproveGridList.getRow(rowKey);
				edsEdms.approveConfirmSubmit(data);
			})
		})
		/**********************************************************************
		 * 화면 이벤트 영역 END
		 ...
		 ***********************************************************************/
		 async function lnitIframe(docDivi){
			var rows = edmsSubmitApproveGridList.getRowCount();
			if(rows > 0){
				let url="";
		...
		async function validateCheck(){
			// 유효성 검사
			var validate = await edmsSubmitApproveGridList.validate();
			for(var i=0; i<validate.length; i++) {
				if(validate[i].errors.length != 0) {
					for (var r = 0; r < validate[i].errors.length; r++) {
						if (validate[i].errors[r].errorCode == "REQUIRED") {
							await edmsSubmitApproveGridList.focus(validate[i].rowKey,validate[i].errors[r].columnName); // 빈 필수 값 포커스
							alert('필수값을 입력해 주세요.')
							return false;
						}
					}
	...
	<div class="row"style="margin: 0;height: 100vh">
		<div class="col-md-12 templateTop">
			...
			<div class="btn-group float-right" style="padding: 3px;">
				<form class="form-inline" role="form" name="edmsSubmitApproveGridListButtonForm" id="edmsSubmitApproveGridListButtonForm" method="post" onsubmit="return false;">
					<button type="button" class="btn btn-sm btn-primary" name="btnSearch" id="btnSearch" onclick="doAction('edmsSubmitApproveGridList', 'search')"><i class="fa fa-search"></i> 조회</button>
				</form>
			</div>
		</div>
		<div class="col-md-4" style="height: calc(100% - 50px);">
			<div id="edmsSubmitApproveGridList" style="height: 100%;">
				<!-- 시트가 될 DIV 객체 -->
				<div id="edmsSubmitApproveGridListDIV" style="width:100%; height:100%;"></div>
			</div>
		</div>
				...
				<form class="form-inline" role="form" name="edmsEstGridItemButtonForm" id="edmsEstGridItemButtonForm" method="post" onsubmit="return false;">
					<div class="container">
							<div class="col text-center">
								<button type="button" class="btn btn-sm btn-success" name="btnConfirm" id="btnConfirm" ><i class="fa fa-check-circle "></i> 읽음처리</button>
							</div>
					</div>
				</form>
			</div>
