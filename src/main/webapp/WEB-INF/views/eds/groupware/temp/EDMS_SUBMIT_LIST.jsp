...
<head>
	...
	<script>
		...
		async function init() {
			...
			/* Button 셋팅 */
			edsUtil.setButtonForm(document.querySelector("#edmsSubmitGridListButtonForm"));
			 ...
			 * Grid Info 영역 START
			 ***********************************************************************/
			edmsSubmitGridList = new tui.Grid({
				el: document.getElementById('edmsSubmitGridListDIV'),
				scrollX: true,
				scrollY: true,
				editingEvent: 'click',
			...
			edmsSubmitGridList.setColumns([
				...
				{ header:'완료',		name:'deadDivi',		minWidth:50,	align:'center',	formatter: 'listItemText',
					editor: {
						type: 'select',
						options: {
							listItems:setCommCode("COM026")
						}
					},
					filter: { type: 'select', showApplyBtn: true, showClearBtn: true } },
				// hidden(숨김)
			...
			//disabled 셋팅
			edmsSubmitGridList.disableColumn('docDivi');
			edmsSubmitGridList.disableColumn('submitDt');
			/**********************************************************************
			 * Grid Info 영역 END
			 ***********************************************************************/
			 ...
			 * Grid 이벤트 영역 START
			 ***********************************************************************/
			edmsSubmitGridList.on('focusChange', async ev => {
				if(ev.rowKey != ev.prevRowKey){
					const docDivi=edmsSubmitGridList.getRow(ev.rowKey).docDivi;
					setTimeout(async function(){
						await lnitIframe(docDivi);
					}, 100);
				}
			});
			edmsSubmitGridList.on('afterChange', ev => {
			...
			edmsSubmitGridList.on('click', async ev => {
				var colNm = ev.columnName;
				var target = ev.targetType;
				var focusDataNm=edmsSubmitGridList.getFocusedCell().columnName;
				if((focusDataNm === 'docDivi')){
					edmsSubmitGridList.finishEditing();
				}
			});
			/**********************************************************************
			 * Grid 이벤트 영역 END
			...
			/* 조회 */
			doAction("edmsSubmitGridList", "search");
		}
		...
		async function doAction(sheetNm, sAction) {
			if (sheetNm == 'edmsSubmitGridList') {
				switch (sAction) {
					case "search":// 조회
						edmsSubmitGridList.clear(); // 데이터 초기화
						var param = ut.serializeObject(document.querySelector("#searchForm")); //조회조건
						await edmsSubmitGridList.resetData(edsUtil.getAjax("/EDMS_SUBMIT_LIST/selectSubmitList", param)); // 데이터 set
						if(edmsSubmitGridList.getRowCount() > 0 ){
							edmsSubmitGridList.focusAt(0, 0, true);
						}
						break;
					case "input":// 신규
						...
						edmsSubmitGridList.appendRow (appendedData, { focus:true }); // 마지막 ROW 추가
						const appendRow=edmsSubmitGridList.getFocusedCell();
						const rows=edmsSubmitGridList.getRow(appendRow.rowKey);
						edmsSubmitGridList.enableCell(rows.rowKey, 'docDivi');
						break;
					case "save"://저장
						await edsUtil.doCUD("/EDMS_SUBMIT_LIST/cudSubmitList", "edmsSubmitGridList", edmsSubmitGridList);// 저장
						break;
					case "delete"://삭제
						await edmsSubmitGridList.removeCheckedRows(true);
						await edsUtil.doCUD("/EDMS_SUBMIT_LIST/cudSubmitList", "edmsSubmitGridList", edmsSubmitGridList);
		...
		$("#iframSubmit").ready(function() {
				...
				window.addEventListener("message", function(message) {
					...
					{
						var row = edmsSubmitGridList.getFocusedCell();
						const data= edmsSubmitGridList.getRow(row.rowKey);//그리드 기준 조회조건
				...
				document.getElementById("btnTempSave").addEventListener("click", async function(message) {
					...
					if(!validate) return;
					edmsSubmitGridList.finishEditing();
					const rowKey=edmsSubmitGridList.getFocusedCell().rowKey;
					const data=edmsSubmitGridList.getRow(rowKey);
		 ...
		 ***********************************************************************/
		 async function lnitIframe(docDivi){
			var rows = edmsSubmitGridList.getRowCount();
			if(rows > 0){
				let url="";
		...
		async function validateCheck(){
			// 유효성 검사
			var validate = await edmsSubmitGridList.validate();
			for(var i=0; i<validate.length; i++) {
				if(validate[i].errors.length != 0) {
					for (var r = 0; r < validate[i].errors.length; r++) {
						if (validate[i].errors[r].errorCode == "REQUIRED") {
							await edmsSubmitGridList.focus(validate[i].rowKey,validate[i].errors[r].columnName); // 빈 필수 값 포커스
							alert('필수값을 입력해 주세요.')
							return false;
						}
					}
	...
	<div class="row"style="margin: 0;height: 100vh">
		<div class="col-md-12 templateTop">
			...
			<div class="btn-group float-right" style="padding: 3px;">
				<form class="form-inline" role="form" name="edmsSubmitGridListButtonForm" id="edmsSubmitGridListButtonForm" method="post" onsubmit="return false;">
					<button type="button" class="btn btn-sm btn-primary" name="btnSearch" id="btnSearch" onclick="doAction('edmsSubmitGridList', 'search')"><i class="fa fa-search"></i> 조회</button>
					<button type="button" class="btn btn-sm btn-primary" name="btnInput" id="btnInput" onclick="doAction('edmsSubmitGridList', 'input')"><i class="fa fa-plus"></i> 신규</button>
					<button type="button" class="btn btn-sm btn-primary" name="btnDelete" id="btnDelete" onclick="doAction('edmsSubmitGridList', 'delete')"><i class="fa fa-trash"></i> 삭제</button>
				</form>
			</div>
		</div>
		<div class="col-md-4" style="height: calc(100% - 50px);">
			<div id="edmsSubmitGridList" style="height: 100%;">
				<!-- 시트가 될 DIV 객체 -->
				<div id="edmsSubmitGridListDIV" style="width:100%; height:100%;"></div>
			</div>
		</div>
