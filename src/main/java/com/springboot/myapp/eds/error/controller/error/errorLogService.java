...
public class errorLogService {
    ...
    @Autowired
    private com.springboot.myapp.eds.error.controller.error.errorLogMapper errorLogMapper;
    ...
    public List<errorLogListVO> selectErrorLogList(Map<String, Object> map) throws Exception {
        ...
        List<errorLogListVO> result = errorLogMapper.selectErrorLogList(map);
        return result;
    }

    public Map<String, Object> cudErrorLogList(Map<String, Object> map) throws Exception {
        ...
        try {
            ...
            switch (status) {
                case "C" -> {
                    errorLogMapper.insertErrorLogList(map);
                    returnData.put("note","성공적으로 내역이 예약되었습니다.");
                    break;
                }
                case "U" -> {
                    errorLogMapper.updateErrorLogList(map);
                    returnData.put("note","성공적으로 내역이 수정되었습니다.");
                    break;
                }
                case "D" -> {
                    errorLogMapper.deleteErrorLogList(map);
