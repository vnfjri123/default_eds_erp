...
public class Util {
     ...
     *
     * <pre>
     * StringUtil.removeColonChar(null)       = null
     * StringUtil.removeColonChar("")         = ""
     * StringUtil.removeColonChar("a:sdfg:qweqe") = "asdfgqweqe"
     * </pre>
     *
     * @param str  입력받는 기준 문자열
     * @return " : "가 제거된 입력문자열
    ...
    /**
     * 원본 문자열의 포함된 특정 문자열 중 마지막 문자열을 새로운 문자열로 변환하는 메서드
     * @param source 원본 문자열
     * @param subject 원본 문자열에 포함된 특정 문자열
     * @param object 변환할 문자열
     * @return sb.toString() 새로운 문자열로 변환된 문자열 / source 특정문자열이 없는 경우 원본 문자열
     */
    public static String replaceLast(String source, String subject, String object) {
    	int pos = source.lastIndexOf(subject);
    	if(pos > -1){
    		return source.substring(0, pos) + object + source.substring(pos + subject.length(), source.length());
    	} else {
    		return source;
    	}
    }
    ...
    public static float zeroConvertFloat(String src) {
    ...
    /*
    public static double DoubleSet(String src, int op) {
    	String dv = "";
    	int opt = 0;
    	
		if (src == null || src.equals("null") || "".equals(src) || " ".equals(src) || "0".equals(src)) {
		    return 0;
		} else {
			if(op == 101 ){  //수량소수점
				opt = Integer.parseInt(loginVO.getOpt1());
			}else if(op == 102 ){ //원화단가 소수점
				opt = Integer.parseInt(loginVO.getOpt2());
			}else if(op == 103 ){ // 외화단가 소수점
				opt = Integer.parseInt(loginVO.getOpt3());
			}else if(op == 104 ){ // 비율소수점
				opt = Integer.parseInt(loginVO.getOpt4());
			}else if(op == 105 ){ // 금액소수점
				opt = Integer.parseInt(loginVO.getOpt5());
			}else if(op == 106 ){ // 외화금액소수점
				opt = Integer.parseInt(loginVO.getOpt6());
			}else if(op == 107 ){ // 환율소수점
				opt = Integer.parseInt(loginVO.getOpt7());
			}
			
			//수량 자리수
			dv =  convertFloor(Double.parseDouble(src.trim()),opt);
...
/**
	...
	public static String addMinusChar(String date) {
		...
		if(date.length() == 8) {
            ...
            return date.substring(0, 4).concat("-").concat(date.substring(4, 6));
        }
		else return "";
	}

    /**
     * <p>숫자 형식의 문자열 내부에 콤마 character(,)를 추가한다.</p>
     *
     * <pre>
     *   StringUtil.addCommaChar("1234567.89") = "1,234,567.89"
     * </pre>
     *
     * @param numericValue  입력받는 문자열
     * @return " , "가 추가된 입력문자열
     */
    public static String addCommaChar(String numericValue) {
        try {
            if (numericValue == null) {
                return "0";
            }
            Double doubleValue = Double.parseDouble(numericValue);
            return NumberFormat.getNumberInstance(Locale.US).format(doubleValue);
        } catch (NumberFormatException e) {
            // 숫자 형식이 아닌 문자열에 대한 처리
            return "0";
        }
    }
	
	/**
     * 입력받은 String 문자열을 Base64형태로 변환한 String값 반환
     *
     * @param source  입력받는 문자열
     * @return String
     */
	public static String base64Encrypt(String source){
		String encStr = Base64.encodeBase64String(source.getBytes());
	...
	/**
     * 입력받은 Base64 형태의 String 문자열을 일반 String값으로 반환
     * 반드시 Base64형태의 String 문자열만 입력해야 한다.
     * @param source  입력받는 문자열
     * @return String
     */
	public static String base64Decrypt(String source){
		String decStr = new String(Base64.decodeBase64(source));
